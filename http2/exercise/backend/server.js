...
// keep track of all streams that are currently connected
let connections = [];
...
server.on('stream', (stream, headers) => {
  // everything is going to both open a stream and make a request, streams open for every req from browser
  ...
  if (path === '/msgs' && method === 'GET') {
    ...
    }) // there is no way to stream json, so type is text
